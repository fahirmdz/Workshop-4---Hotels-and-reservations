...
public: 
	...
	Usluga(const char* naziv, int cijena, TipNaplate tipNaplate) : _cijena(cijena), _tipNaplate(tipNaplate) {
		_naziv = AlocirajNizKaraktera(naziv);
	}

~Usluga() { delete[] _naziv;   _naziv = nullptr; }

void Info() { cout << "Naziv usluge: " << _naziv << endl;   cout << "Cijena: " << _cijena << endl; }
};
...
public: 
	Smjestaj(int broj, TipSmjestaja tip, int maxBrojOsoba, double cijenaPoOsobi) :   _broj(broj),
		_tipSmjestaja(new TipSmjestaja(tip)),_maxBrojOsoba(maxBrojOsoba), _cijenaPoOsobi(cijenaPoOsobi)  {  } 

	~Smjestaj() { delete _tipSmjestaja;   _tipSmjestaja = nullptr; }
...
void Info() { cout << "Ime i prezime: " << _imePrezime << endl;  
...
class Hotel {
	char* _naziv;   
	Smjestaj* _smjestaji;  //Ponuda smještaja 
	int _brojSmjestaja;  
	Rezervacija* _rezervacije;//Rezervacije hotela (protekle, aktivne i nadolazece) 
...
public:  
	Hotel(const char* naziv) : _smjestaji(nullptr), _brojSmjestaja(0),  
		_rezervacije(nullptr), _brojRezervacija(0)  {   _naziv = AlocirajNizKaraktera(naziv);  } 
...
#pragma region Hotel 
	...
	hotel1.DodajSmjestaj(soba3); //Nece se dodati radi zauzetosti broja smještaja 
...
#pragma region RezervacijaGost1 
	...
	//Posljednji parametar konstruktora je broj osoba 
	...
	/*Vrši ispis racuna sa svim stavkama rezervacije (posebno stavka za smještaj, a posebno za sve usluge sa 
...
#pragma endregion 
	...
	//Pronalazi i vraca podatke o najtraženijem smještaju hotela (onaj koji je najviše puta rezervisan u datom periodu)
